...
class SupportVectorMachineClassifier(classificationMethod.ClassificationMethod):
  ...
  def train(self, trainingData, trainingLabels, validationData, validationLabels):
    ...
    self.clf = grid_search.GridSearchCV(svr, parameters)
    tdata = self.formattingData(trainingData, self.features, None, None)
    X = tdata
    #print(tdata)
    Y = trainingLabels
    ...
    #data = self.formattingData(trainingData, self.features, None, None)
    #label = self.legalLabels
    self.clf.fit(X, Y)
    #clf.score(tdata, trainingLabels)
    
    #print(validationLabels)
    #print(clf.score(X,Y))
    #self.myclassifier = clf.predict(X)
    
    #print(Y)
    #print(self.myclassifier)
    
    
    #print(clf.predict(X1))
    #print(self.myclassfier.predict(X1))
    
    #self.classify(X)
    #for x in self.classifer.predict(X)
    #self.weight[
    #print(trainingLabels[x])
    #print(self.myclassfier.predict(X)[x])
    ...
    #method fit, grab sym always linear and call fit on it
  ...
  def classify(self, testData):
    ...
    #if self.features
    test = self.formattingData(testData, self.features, None, None)
