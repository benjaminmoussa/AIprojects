...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #first check if no noisyDistance, thus the ghost has been captured
    if noisyDistance != None:
        #counter for new updated beliefs of ghost locations
        ghostmove = util.Counter()
        #get list of possible ghost positions
        #plist = self.legalPositions()
        for x in self.legalPositions:
            estdist = util.manhattanDistance(x, pacmanPosition)
            #x is possible position, get manhattan distance to this location
            #print self.beliefs[x]
            #print("meow")
            #print emissionModel[estdist]
            #update prob of this location by multiplying beliefs*prob distribution of this location
            ghostmove[x] = self.beliefs[x] * emissionModel[estdist]
        #update beliefs
        ghostmove.normalize()
        self.beliefs = ghostmove

  
    else:
        jailpos = util.Counter()
        position = self.getJailPosition()
        #update that the ghost is in jail
        jailpos[position] = 1
        #update beliefs
        jailpos.normalize()
        self.beliefs = jailpos
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    updatebelief = util.Counter()
    #iterate through all possible ghost positions
    for x in self.legalPositions:
        #x's are all possible old positions, so update new position distribution based on all old ones
        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, x))
        #loop over key value pairs in the new distribution, get probaility of new position given old x and update probabilities of new position
        for newPos, prob in newPosDist.items():
            #print(self.beliefs[x])
            #update belief (prob of new position) based on prob of old position*prob of new position
            updatebelief[newPos] += self.beliefs[x]*prob
    updatebelief.normalize()
    self.beliefs = updatebelief
